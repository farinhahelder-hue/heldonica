...
- **Côté cerveau, correctif « n'invente rien » (25/09 soir, code dans le dossier `heldonica-brain`, hors dépôt)** : le chat répondait des récits inventés (lieux, prix, GPS imaginaires) car il utilisait le prompt « rédacteur blog ». Nouveau `CHAT_SYSTEM_PROMPT` (répondre à la vraie question, ne jamais inventer, réponses courtes), branché sur le pont CMS + `/api/chat`, vérifié en production après redémarrage du service (`HeldonicaBrain`, worker 25216). Détail dans le « Journal des modifications » du README du cerveau.
