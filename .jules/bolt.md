...
## 2024-05-18 - Promise.all for independent server fetch calls
**Learning:** Sequential `await` calls in Next.js Server Components for independent data sources (like fetching settings, posts, and page content) block the execution thread unnecessarily and delay the Time To First Byte (TTFB).
**Action:** Always refactor independent asynchronous calls into a single `Promise.all` batch to fetch data concurrently, optimizing the server-side rendering performance.
