...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: 'GEMINI_API_KEY manquante' }, { status: 503 });
  }
  ...
  try {
    ...
    await logAiRequest({
      ...
      model: MODEL,
      promptPreview: `Mode: ${mode} | ${message.slice(0, 100)}`,
      statusCode: 502,
      durationMs,
      error: msg,
    });
  ...
  if (sb) {
    ...
    if (error) {
      if (error.code !== 'PGRST205' && error.code !== '42P01') {
        console.warn('[ai-copilot] historique non enregistré :', error.message);
      }
    } else {
      enregistre = true;
    }
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    ...
    console.error('[ai-copilot] lecture historique :', error.message);
