import type { Metadata } from 'next'
import Header from '@/components/Header'
import Footer from '@/components/Footer'
import SubDestinationTemplate from '@/components/SubDestinationTemplate'
import InlineEditProvider from '@/components/inline-edit/InlineEditProvider'
import { getPageZones } from '@/lib/cms-zones'
import { buildPageMetadata } from '@/lib/page-metadata'

const metadata: Metadata = {
  title: "Cagliari en couple : notre carnet slow travel | Heldonica",
  description: "Le sud. Lagune, dunes, sel en Sardaigne. Notre guide slow travel testé en couple : pépites locales, adresses insolites et conseils pratiques.",
  openGraph: {
    title: "Cagliari en couple : notre carnet slow travel | Heldonica",
    description: "Le sud. Lagune, dunes, sel en Sardaigne. Notre guide slow travel testé en couple : pépites locales, adresses insolites et conseils pratiques.",
    type: 'website',
    images: ['/og-default.jpg'],
    locale: 'fr_FR',
    siteName: 'Heldonica'
  },
  alternates: {
    canonical: "https://www.heldonica.fr/destinations/sardaigne/cagliari"
  }
}

export async function generateMetadata(): Promise<Metadata> {
  return buildPageMetadata('destinations-sardaigne-cagliari', metadata)
}


const highlights = [
  {
    emoji: '🦩',
    title: 'Lagune',
    description: 'Flamants roses.',
  },
  {
    emoji: '🏖️',
    title: 'Piscinas',
    description: 'Dunes.',
  },
  {
    emoji: '🧂',
    title: 'Salines',
    description: 'Sel rose.',
  }
]

export default async function CagliariPage() {
  const zones = await getPageZones('destinations-sardaigne-cagliari')
  return (
    <InlineEditProvider page="destinations-sardaigne-cagliari" initialZones={zones}>
      <Header />
      <SubDestinationTemplate
        page="destinations-sardaigne-cagliari"
        name="Cagliari"
        parentName="Sardaigne"
        parentSlug="sardaigne"
        heroImage="/og-default.jpg"
        introText="Cagliari, c'est le sud. La lagune avec les flamants, les dunes de Piscinas."
        highlights={highlights}
        localTip="Prends le temps de visiter les lieux d'intérêt en début de matinée et d'échanger avec les habitants pour dénicher les meilleures adresses de quartier."
      />
      <Footer />
    </InlineEditProvider>
  )
}
