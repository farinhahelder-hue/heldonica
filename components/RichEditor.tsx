...
export default function RichEditor({ value, onChange, placeholder = 'Contenu de l'article…', cmsPassword, onImageUpload }: Props) {
  const editorRef = useRef<HTMLDivElement>(null);
  const lastHtml = useRef(value);

  // Init content only once on mount
  useEffect(() => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (data.url) {
      ...
      // Style inline pour l'image insérée
  ...
  return (
    <div style={{ border: '1.5px solid #e0dbd5', borderRadius: '.75rem', overflow: 'hidden', background: '#faf9f7' }}>
      ...
      <div style={{
        ...
        <label title="Insérer une image" style={{
          padding: '.3rem .55rem', border: '1px solid transparent',
          borderRadius: '.35rem', background: 'none', cursor: 'pointer',
          fontSize: '.88rem', color: '#444', lineHeight: 1,
        }}
          onMouseEnter={e => (e.currentTarget.style.background = '#f0ece6')}
          onMouseLeave={e => (e.currentTarget.style.background = 'none')}
        >
      ...
      <style>{`
        ...
        [contenteditable] blockquote {
          border-left: 3px solid #d4a88a; margin: 1em 0;
          padding: .5em 1em; background: #faf3ee; color: #666;
          border-radius: 0 .5rem .5rem 0; font-style: italic;
        }
