...
import { Save, Globe, Palette, Share2, Search, FileText, Wrench } from 'lucide-react';
...
export default function CmsSettingsPanel() {
  ...
  useEffect(() => {
    ...
    fetch('/api/cms/settings')
      .then(r => r.json())
      .then(data => {
        // Support both { key: value } flat object and { settings: [...] } array
        if (data && typeof data === 'object' && !Array.isArray(data)) {
          const flat: Record<string, string> = {};
          // If it has a settings array, merge it
          if (Array.isArray(data.settings)) {
            data.settings.forEach((s: Setting) => { flat[s.key] = s.value ?? ''; });
          }
          // Also merge flat keys (skip 'settings' key itself)
    ...
    <div className="flex gap-6 h-full">
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          {loading && (
            <div className="text-sm text-gray-400 py-8 text-center">Chargement des paramètres…</div>
          )}
