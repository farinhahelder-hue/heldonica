  ...
  return (
    <div className="flex gap-6">
      <div className="w-96 shrink-0 space-y-4">
        <div className="flex flex-wrap gap-1.5 bg-gray-100/80 rounded-xl p-1.5">
          {TABS.map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-grow sm:flex-grow-0 flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-all duration-200 ${
                activeTab === tab.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-white/40'
              }`}
            >
              {tabIcons[tab.id]}
              {tab.label}
