-- Migration: cms_pillar_pages — Données destinations pilier (version idempotente)
-- Date: 2026-07-30
-- Remplace: 20260719_create_cms_pillar_pages.sql (jamais appliquée en prod)
--
-- Corrections apportées vs 20260719 :
--   1. Policies rendues idempotentes (DROP POLICY IF EXISTS) — l'originale
--      plantait au second passage.
--   2. Images du seed : les 3 URLs de l'originale (2 URLs WordPress legacy
--      + 1 URL Unsplash) sont remplacées par '/og-default.jpg', c'est-à-dire
--      la valeur exacte servie aujourd'hui en prod par lib/pillar-data.ts.
--      Appliquer l'originale aurait réintroduit du stock Unsplash, contre les
--      décisions A2.
--   3. ON CONFLICT DO UPDATE -> DO NOTHING : une fois la ligne créée, le CMS
--      en est maître. La migration ne réécrase jamais un contenu édité.
--
-- Le contenu textuel (intro, itinéraires, FAQ, verdicts) est repris à
-- l'identique de 20260719, lui-même identique à lib/pillar-data.ts.
